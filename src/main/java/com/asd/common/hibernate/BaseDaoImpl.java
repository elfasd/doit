...
public class BaseDaoImpl<T> implements BaseDao<T> {
     ...
     */
    @Override
    public List<T> findByPropertyLike(Map<String, Object> conditionMap) {
        StringBuilder hql = new StringBuilder();
        hql.append("from  " + entityClazz.getSimpleName());
        if (!conditionMap.isEmpty()) {
            Iterator<String> it = conditionMap.keySet().iterator();
            String key = it.next();
            hql.append(" where  " + key + " like:" + key);
            while (it.hasNext()) {
                key = it.next();
                hql.append(" and  " + key + " like:" + key);
            }
        }
        return findList(hql.toString(), conditionMap);
    }
     ...
     */
    @Override
    public List<T> findByPropertyOrder(Map<String, Object> conditionMap, Map<String, Object> orderMap) {
        StringBuilder hql = new StringBuilder();
        hql.append("from  " + entityClazz.getSimpleName());
        ...
        if (!orderMap.isEmpty()) {
            Iterator<String> it1 = orderMap.keySet().iterator();
            String key = it1.next();
            hql.append(" order by   " + key + " " + orderMap.get(key));
            while (it1.hasNext()) {
                key = it1.next();
                hql.append("," + key + " " + orderMap.get(key));
            }
        }
     ...
     */
    @Override
    public <T1> Pagination<T1> findPaginationOrder(Map<String, Object> params, Map<String, Object> orderMap,
                                                   int pageIndex, int pageSize) {

        StringBuilder queryString = new StringBuilder();
        queryString.append("from  " + entityClazz.getSimpleName());
        ...
        //在informix等数据库中count(*)的where查询条件后不能有order by
        String quaryStringNoOrder = queryString.toString();

        if (!orderMap.isEmpty()) {
            Iterator<String> it1 = orderMap.keySet().iterator();
            String key = it1.next();
            queryString.append(" order by  " + key + " " + orderMap.get(key));
            while (it1.hasNext()) {
                key = it1.next();
                queryString.append("," + key + " " + orderMap.get(key));
            }
        }
        ...
        if ((pageSize > 0) && (pageIndex > 0)) {
            String hql = parseSelectCount(quaryStringNoOrder);
            rowsCount = ((Long) get(hql, params)).longValue();
        } else {
            rowsCount = items.size();
        }
    ...
    @Override
    public <T1> List<T1> findList(CharSequence queryString) {
        Query query = getSession().createQuery(queryString.toString());
        return query.list();
    }
